...
public class GoalService {
    ...
    public Goal quickUpdateGoal(Long goalId, BigDecimal addAmount, BigDecimal newTargetAmount, LocalDate newTargetDate,
            Long userId) {
        Goal goal = getGoalById(goalId, userId)
                .orElseThrow(() -> new RuntimeException("Goal not found"));
        ...
        if (addAmount != null && addAmount.compareTo(BigDecimal.ZERO) > 0) {
            goal.setCurrentAmount(goal.getCurrentAmount().add(addAmount));
        }

        // Update target amount if provided
        if (newTargetAmount != null && newTargetAmount.compareTo(BigDecimal.ZERO) > 0) {
